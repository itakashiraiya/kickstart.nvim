...
local plugin_path = { "custom", "lazy", "plugins" }
local dir = vim.fn.stdpath("config") .. "/lua/" .. table.concat(plugin_path, "/")
local base_plugin = table.concat(plugin_path, ".")
for plugin, type in vim.fs.dir(dir) do
  if type == 'file' and plugin:sub(-4) == ".lua" then
    table.insert(plugins, require(base_plugin .. "." .. plugin:gsub("%.lua$", "")))
  end
end
